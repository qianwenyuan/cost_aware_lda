...
def acq_max(ac, gp, y_max, bounds, random_state, alpha=1.0, cost_func=None, n_warmup=100000, n_iter=250):
    ...
    #     x_max = res.x
    #
    # print("Trial %s, acq %.5f, max_acq: %.5f" % (res.x.__repr__(), -res_max_acq, max_acq))
    # print("Trial %s, acq %.5f" % (x_max.__repr__(), -res_max_acq))
    # print("Trial %s, acq %.5f" % (np.clip(x_max, bounds[:, 0], bounds[:, 1]).__repr__(), -res_max_acq))
