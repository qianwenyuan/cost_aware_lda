...
def bo_target(f):
    def bo_f(**params):
        ...
        x = np.zeros(dim)
        for i in range(dim):
            x[i] = int(params["x%d" % i])
        return -f(x)
    return bo_f

def run_bo(target_f, pbounds, n_iter=30, init_params=None):
    # bayesian optimization search
    print
    bo_target_f = bo_target(target_f)
    bo = BayesianOptimization(bo_target_f, pbounds)
    init_points = 2
...
def run_bo_cost(target_f, pbounds, n_iter=30, init_params=None, acq_type='div'):
    print
    bo_target_f = bo_target(target_f)
    # cost = virtual_cost
    bo = BayesianOptimization(bo_target_f, pbounds, cost_function=virtual_cost)
    init_points = 2
    if init_params is not None:
        bo.init_with_params(init_params)
...
def main():
    ...
    for i in range(n_runs):
        ...
        x_ex, y_ex = run_ex(lda_train, n_iter)
        x_bo_cost_my, y_bo_cost_my, bo_cost_my = run_bo_cost(lda_train, pbounds, n_iter, _init_params, 'others')
        x_bo_cost_div, y_bo_cost_div, bo_cost_div = run_bo_cost(lda_train, pbounds, n_iter, _init_params, 'div')
        x_bo, y_bo, bo = run_bo(lda_train, pbounds, n_iter, _init_params)
        plot_start = init_points
        plt.plot(x_ex[0:], y_ex[0:], marker="1", label="ex_search")
        plt.plot(x_bo[0:], y_bo[0:], marker="x", label="bo")
        # plt.scatter(x_bo[plot_start:], bo.space.Y[plot_start:], marker="x", label="bo_tria")
        plt.plot(x_bo_cost_div[0:], y_bo_cost_div[0:], marker="+", label="bo_cost_aware_div")
        plt.plot(x_bo_cost_my[0:], y_bo_cost_my[0:], marker="^", label="bo_cost_aware_my")
        # plt.scatter(x_bo_cost[plot_start:], bo_cost.space.Y[plot_start:], marker="o", label="bo_cost_aware_trial")
        # plt.ylim(-5, 0)
        plt.legend()
        plt.savefig('./1_lda_cost_awre_BO_compare_tf{}_{}.png'.format(costtype,i))
        plt.close()
        ...
        plt.legend()
        plt.savefig('./1_lda_cost_awre_BO_compare_cost{}_{}.png'.format(costtype,i))
        plt.close()
        #plt.show()
